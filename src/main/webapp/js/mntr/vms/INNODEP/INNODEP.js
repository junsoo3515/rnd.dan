...
function initInnodep(iWidth, iHeight) {
    ...
    try {
        result = innodep.CreateInstance(); // success : 1, failure : 0
    } catch (e) {
...
function initInnodep2(iWidth, iHeight) {
    var object = $('<object/>', {
        id: 'innodep',
        name: 'innodep',
...
function initInnodepCastnet(iWidth, iHeight) {
    ...
    if (isInnodepInit) {
        if (innodep != null && typeof innodep != 'undefined') {
            ...
            if (typeof canAudio != 'undefined' && canAudio) {
                innodep.SoundOnOff(0);
            }
            if (typeof canMic != 'undefined' && canMic) {
                innodep.SendAudio(0);
            }
